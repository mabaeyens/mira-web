...
- **Delete the leftover Tailwind files** (`assets/tailwind.css`, `tailwind.config.js`, `tailwind-input.css`). No page links them any more (`vera/privacy` was rebuilt on the shared `style.css` on 2026-08-27); they only survive because deleting needs a deliberate call.
...
- [2026-09-05] Brought the whole site onto the homepage's night bento skin: `style.css` (shared by `/mira`, `/vera`, `/aura` and the privacy/support pages) now carries the cool near-black and light tokens, Newsreader + Manrope + IBM Plex Mono, pill nav, round icon buttons, pill CTAs (accent-filled primary), and tile-style cards instead of hairline grids; the grain overlay is gone and hero highlights are italic in the accent. Legal pages' inline blocks moved to the shared font tokens. Also: the hero number stream runs ~25% faster, and the Aura tile's phone now peeks in from the bottom so the copy no longer covers it.
...
- Design direction: the whole site (since 2026-09-05) is the "night bento" look, serif display over a cool near-black with a light sibling, tiles that show the apps, honest copy. The homepage has it in `home.css`; every other page gets it from the shared `style.css`. The old "field manual" system (warm charcoal, grain, Lora + Inter) is gone.
...
- The homepage loads only `home.css` (self-contained tokens, header, hero, stream, bento, principles, footer). `/mira`, `/vera` and `/aura` load the shared `style.css` plus their own sheet; the two sheets share token values by hand, so a palette or font change has to be made in both. Nothing links `assets/tailwind.css` any more (see the Pending item to delete it).
